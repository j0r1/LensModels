...
iws = inversion.InversionWorkSpace(z_lens, 250*ANGLE_ARCSEC, cosmology=cosm)
Dd = cosm.getAngularDiameterDistance(z_lens)

imgList = images.readInputImagesFile("images.txt", True) 
for i in imgList:
    iws.addImageDataToList(i["imgdata"], i["z"], "pointgroupimages") # Not the best name, is the RMS based fitness measure
    iws.addImageDataToList(i["imgdata"], i["z"], "pointimages")

iws.setDefaultInversionArguments(sheetSearch = "genome", maximumGenerations=2)
...
def setBasisFunctions(lens, minSub, maxSub):
    ...
    # Add SIS basis lens (weight will also be optimized by GA)
    iws.addBasisFunctions( [{
        "lens": lenses.SISLens(Dd, { "velocityDispersion": 100000 }),
        "center": [ -4.8*ANGLE_ARCSEC, -18.1*ANGLE_ARCSEC ]
    }])
